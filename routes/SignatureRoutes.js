...
router.post('/',async (req,res,next)=>{
    try{
        ...
        const newsignature = await signatureSchema.create({...req.body,user:req.user.clerkUserId})
        if(!newsignature){
...
router.get('/',async (req,res,next)=>{
    try{
        const signatures = await signatureSchema.find({user:req.user.clerkUserId}) || {}
        if(!signatures){
...
router.get('/:id', async(req,res,next)=>{
    try{
        const signatures = await signatureSchema.findOne({_id:req.params.id,user:req.user.clerkUserId}) || {}
        if(!signatures){
            return res.status(404).json({message:'No signatures found'})
        }
        res.status(200).json(signatures)
...
router.put('/edit/:id', async(req,res,next)=>{
    try{
        ...
        const updatedSignature =  await signatureSchema.findOneAndUpdate({_id:req.params.id,user:req.user.clerkUserId},{new:true},{...req.body})
        if(!updatedSignature){
...
router.delete('/:id', async(req,res,next)=>{
    try{
        const deletedSignature =  await signatureSchema.findOneAndDelete({_id:req.params.id,user:req.user.clerkUserId})
        if(!deletedSignature){
